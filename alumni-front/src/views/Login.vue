...
<v-app>
  <v-dialog v-model="dialog" persistent max-width="600px" min-width="360px">
            <div>
                <v-tabs v-model="tab" show-arrows background-color="blue darken-4" icons-and-text dark grow>
  ...
  methods: {
    ...
    validate() {
      if (this.$refs.loginForm.validate()) {
        this.signIn({email:this.loginEmail, password:this.loginPassword}).then(()=> {
                this.$router.replace({
                    name: 'Dashboard'
                })
            }).catch(() =>{
      ...
      if (this.$refs.registerForm.validate()) {
          this.register({firstname:this.firstname, lastname:this.lastname, email: this.email, password:this.password, password_confirmation: this.password_confirmation}).then (() =>{
